...
- ระบบมี **2 ชั้น แยกหน้าที่กันชัด**:
  ...
  |---|---|---|
  | 🔔 **CATCH** (structured events) | เหตุการณ์อันตราย + ระดับความรุนแรง (precompute + ตรวจเฟรมจริงแล้ว) | **แม่นกว่า narration มาก** — ดูตัวเลขจริงข้อ 10 |
...
| ตรวจมือ | **VLM** | ตอบแค่ "มีมือในกรอบนี้ไหม" → **validate แล้วตรง 8/8, false positive = 0** (ฐานเล็ก: 4 จุดเวลา) |
...
## 10. ความแม่นยำจริงของระบบ (อย่าเคลมเกินนี้ใน UI)

วัดกับเฉลยที่ไล่ตรวจเฟรมจริงครบทุกจุด (`claude_gt_v4.json`) — 74 จุดตรวจ พบการละเมิดจริง **56 จุด**

| อุปกรณ์ | ตรวจพบ | แจ้งผิด | หมายเหตุ |
|---|---|---|---|
| หมวก | **100% (14/14)** | **0** | เห็นชัด ตรวจแม่นที่สุด |
| ถุงมือ | **57% (4/7)** | 3 | พลาดช่วง 02:09–02:15 (ถอดข้างเดียว) · แจ้งผิดตอนผ้าแดงบังมือ |
| หน้ากาก (super-resolution) | **80% (28/35)** | 2 | แจ้งผิดตอนก้มหน้าจนมองไม่เห็น |
| **รวม** | **82% (46/56)** | **5** | |

**สิ่งที่ห้ามเขียนใน UI / สไลด์**
- ❌ "แม่นยำ 100%" — จริงเฉพาะหมวก
- ❌ "ไม่มีการแจ้งผิดเลย" — มี 5 จุด และอธิบายได้ทุกจุด
- ✅ เขียนได้: "ตรวจพบ 82% ของการละเมิดทั้งหมด · หมวกแม่น 100% · ถุงมือกับหน้ากากยังพลาดได้เมื่อถูกบัง"

> ที่มาของตัวเลขและคำสั่งทำซ้ำทั้งหมดอยู่ใน `MEASUREMENTS.md`

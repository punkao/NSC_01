...
<div class="left">
  ...
  <div class="h">CAM-A05 · narration per-second จาก JSON · 0 GPU · CATCH + evidence</div>
...
v.addEventListener('timeupdate',()=>{
  ...
  for(const c of EV){ if(t>=c.t_start){ const k=c.type+c.description; if(!shown.has(k)){shown.add(k);
    ...
    d.innerHTML='<img src="/frames/'+c.evidence_frame+'" onclick="window.open(this.src)">'
      +'<div><span class="t">'+c.mmss+'</span> <span class="ty">'+c.label+' · '+(c.severity||'').toUpperCase()+'</span>'
